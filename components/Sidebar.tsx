...
export default function Sidebar({ dark, onToggleTheme }: SidebarProps) {
  ...
  function isActiveItem(item: NavItem) {
    ...
    if (item.to === '/mealplans') {
      return pathname.startsWith('/mealplans');
    }
    return pathname === item.to;
  }

  // <SignedIn> was removed in Clerk Core 3. The sidebar is a client
  // component that already reads the session for the user card, so it gates
  // on isSignedIn directly. Rendering nothing while Clerk loads avoids a
  // signed-out flash of the nav.
